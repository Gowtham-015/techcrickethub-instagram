...
def test_telegram_isolation_strict_phase14():
...
def test_config_phase14_defaults():
    config = Config.load_from_env(validate=False)
    assert config.final_duplicate_gate_enabled is True
    assert config.publish_lock_enabled is True
    assert config.published_history_enabled is True
    assert config.cloud_runtime_enabled is True
    assert config.final_title_similarity_threshold == 0.65
