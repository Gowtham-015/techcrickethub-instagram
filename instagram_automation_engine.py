...
class InstagramAutomationEngine:
    ...
    def prepare_media(self) -> Dict[str, Any]:
        ...
        # C. ORIGINAL_GENERATED Reel Fallback (when no reusable video candidate exists, generate 1 Reel from fresh news context)
        if not candidate_pool and news_items:
            for n_item in news_items:
                n_cat = (n_item.get("category") or "cricket").strip().lower()
                n_title = n_item.get("title") or ""
                n_summary = n_item.get("summary") or n_title
                ...
                # Skip if story was already published
                norm_n_title = self.final_publish_guard.normalize_text(n_title)
                is_story_published = False
                for pub_item in published_history:
                    pub_norm_t = self.final_publish_guard.normalize_text(pub_item.get("title", ""))
                    if norm_n_title and pub_norm_t and (norm_n_title == pub_norm_t or self.repetition_guard.calculate_similarity(n_title, pub_item.get("title", "")) >= 0.55):
                        is_story_published = True
                        break
                    if n_url and pub_item.get("canonical_source_url") and self.final_publish_guard.canonicalize_url(n_url) == self.final_publish_guard.canonicalize_url(pub_item["canonical_source_url"]):
                        is_story_published = True
                        break
                if is_story_published:
                    continue

                # Generate Reel from fresh news story (max 1 per cycle)
                from instagram_reel_generator import InstagramReelGenerator
                from instagram_public_media_host import PublicMediaHost, normalize_reel_filename

                hasher = hashlib.sha256(f"gennews:{n_url}:{n_title}".encode("utf-8"))
                stable_cid = f"generated-news-reel-{hasher.hexdigest()[:16]}"
                gen_item = dict(n_item)
                gen_item["content_id"] = stable_cid
                gen_item["title"] = n_title
                gen_item["summary"] = n_summary
                gen_item["category"] = n_cat

                gen_dir = os.path.join(self.data_dir, "generated_reels")
                gen_res = InstagramReelGenerator(output_dir=gen_dir).generate_reel_from_facts(gen_item, duration_sec=9.0)
                if gen_res.get("success") and gen_res.get("reel_path") and os.path.exists(gen_res["reel_path"]):
                    raw_path = gen_res["reel_path"]
                    norm_path = os.path.join(os.path.dirname(raw_path), normalize_reel_filename(os.path.basename(raw_path)))
                    if raw_path != norm_path and not os.path.exists(norm_path):
                        try:
                            os.rename(raw_path, norm_path)
                        except Exception:
                            pass
                    target_local = norm_path if os.path.exists(norm_path) else raw_path
                    public_u = PublicMediaHost().upload_video(target_local)
                    evidence_url = "https://raw.githubusercontent.com/Gowtham-015/techcrickethub-instagram/main/instagram_reel_generator.py"

                    candidate_pool.append({
                        "content_id": stable_cid,
                        "title": n_title,
                        "summary": n_summary,
                        "category": n_cat,
                        "media_type": "REEL",
                        "source_name": n_item.get("source_name") or "NewsGenerator",
                        "source_url": n_url,
                        "information_source_url": n_url,
                        "media_source_url": public_u,
                        "video_url": public_u,
                        "local_path": target_local,
                        "local_video_path": target_local,
                        "source_domain": n_item.get("source_domain") or "news.local",
                        ...
                        "rights_evidence": "Original Reel generated by TechCricketHub from fresh factual content using internal Reel generator; no third-party video footage used.",
                        "rights_evidence_url": evidence_url,
                        "license_url": evidence_url,
                        "license": "Original content generated by TechCricketHub",
    ...
    def publish_prepared(self) -> Dict[str, Any]:
        ...
        # 1. Final Pre-Publish Duplicate Guard Check
        g_res = self.final_publish_guard.verify_and_guard(bundle)
        ...
        # 3. Verify external public accessibility
